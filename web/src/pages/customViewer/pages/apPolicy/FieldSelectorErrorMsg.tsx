    ...
    return (
        <div>
            ...
            >
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-2">Select a Proposal No</h3>
                <p className="text-slate-500 max-w-sm font-medium">
                    Please complete the selection above to view the associated documents for the proposal.
                </p>

                <div className="mt-8 flex gap-3">
                    <div className={`p-2 rounded-lg flex items-center gap-2 border transition-all ${filterData.officeCode ? 'bg-green-50 border-green-100 text-green-700' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
                    ...
                    <div className={`p-2 rounded-lg flex items-center gap-2 border transition-all ${filterData.year ? 'bg-green-50 border-green-100 text-green-700' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
                        <BiCheckCircle className={`w-4 h-4 ${filterData.year ? 'text-green-600' : 'text-slate-300'}`} />
                        <span className="text-xs font-bold ">Year</span>
                    </div>
                    <div className={`p-2 rounded-lg flex items-center gap-2 border transition-all ${filterData.propno ? 'bg-green-50 border-green-100 text-green-700' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
