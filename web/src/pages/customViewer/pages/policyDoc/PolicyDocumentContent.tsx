...
import { FileListViewer } from 'src/pages/policySearch/section/FileListViewer';
import { FileViewer } from 'src/pages/policySearch/section/FileViewer';
import { PolicyData } from 'src/pages/policySearch/section/PolicyData';
import { PolicyHeaderSection } from 'src/pages/policyResult/section/PolicyHeaderSection';
...
const PolicyDocumentContent = (props: IOptions) => {
    const { policyId, policyData } = props;
    ...
    const endpoint = StringFormat(ServiceEndpoint.policy.searchPolicyByIdApi, { policyId });
    const filePoint = StringFormat(ServiceEndpoint.policy.getFileApi, { policyId, fileId: selectedFile?.pdfFiles?.id });
    const fileDetailPoint = StringFormat(ServiceEndpoint.policy.getFileDetailApi, { policyId, fileId: selectedFile?.pdfFiles?.id });
    ...
    useEffect(() => {
        const handle = topic.subscribe('fileUpload', (_t: string, d: any) => {
            if (d) {
                dataFromUploadEvent = d;
                handleFetch();
            }
        });
        return () => {
    ...
    return (
        ...
        transition-all duration-300 ease-in-out gap-4 px-5 mx-auto w-full h-[calc(100vh-25px)]">
            <div className="policy-sec overflow-y-auto bg-gray/5 backdrop-blur-xl rounded-2xl border border-gray-200 flex flex-col overflow-hidden">
                ...
                <div className="flex-1  mt-2 p-3">
                    <FileListViewer data={data} policyId={policyId} selectedFile={selectedFile} setSelectedFile={setSelectedFile} />
                </div>
                ...
                <Modal opened={opened} onClose={close} onKeyDown={handleKeyAction("Escape", close)}
                    ...
                    <FileDropZone onClose={close} policyId={policyId} />
                </Modal>
            </div>
